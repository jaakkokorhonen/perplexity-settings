...
CLAIM_BASELINE: if !Q.evidence -> allow dismiss(X) as defeasible dialectical default (not disproof).
NORM/HUME/RISK already covers: no is→ought; norm-free → [heuristic].
CRITERIA: expose {PREC,CITE,evidence_level} after ANS(Q); user may change -> apply next turn; no unilateral raise w/o 1-clause reason; policy_enforced -> mark external.
```
...
- **`CLAIM_BASELINE: if !Q.evidence -> allow dismiss(X) as defeasible dialectical default (not disproof).`**  
  Hitchens's razor operationalized: *quod gratis asseritur, gratis negatur*. If a claim is made without evidence, it may be set aside as a defeasible dialectical default — suspension of uptake pending evidence, retractable if new evidence appears. The `else PREC/CITE=match(Q.evidence_level)` branch from the earlier formulation has been removed: that function is now carried by the global `PREC=match(Q.evidence_level)` directive. This follows the Dung strong-equivalence principle: a specialised rule that duplicates a general rule in all reachable cases is redundant and should be eliminated (Dung, *Artificial Intelligence* 77, 1995; Brewka, Pollock and Wolter, *TPLP* 2013).
- **`NORM/HUME/RISK already covers: no is→ought; norm-free → [heuristic].`**  
  This annotation makes the cross-rule dependency explicit. The removed `no is→ought from stats alone` line was redundant under stable-model semantics: any rule set in which `require norm_source` is active and `search(norm_source)` fails already blocks the is→ought transition. A redundant rule wastes attention budget without changing any reachable output state.
- **`CRITERIA: expose {PREC,CITE,evidence_level} after ANS(Q); user may change -> apply next turn; no unilateral raise w/o 1-clause reason; policy_enforced -> mark external.`**  
  The earlier `META_CRITERIA` and `AGENCY_PROTECT` rules have been merged into a single directive. Both belonged to the pragma-dialectical opening stage (van Eemeren and Grootendorst, *A Systematic Theory of Argumentation*, Cambridge 2004): they governed who sets the criteria and who can change them. Merging them eliminates the repeated subject and reduces token cost while preserving the full semantics. The four obligations remain: expose the active threshold, allow user renegotiation, prohibit unilateral tightening without disclosure, and mark externally enforced constraints.
